...
export interface BaseNodeProps<Model extends BaseNodeModel> {
	node: Model;
  engine: DiagramEngine;
...
export const BaseNodeWidget = <T extends BaseNodeModel>(props: BaseNodeProps<T>) => {
  ...
  return (
    ...
    >
      ...
      <Content>
        <PortsContainer>
          {props.node.getInPorts().map(port => <NodePort left engine={props.engine} port={port} key={port.getID()} />)}
        ...
        <PortsContainer>
          {props.node.getOutPorts().map(port => <NodePort left={false} engine={props.engine} port={port} key={port.getID()} />)}
